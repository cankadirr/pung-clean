...
const commentSectionPath = path.join(__dirname, 'frontend', 'components', 'comments', 'AdvancedCommentSection.tsx');
...
export default function AdvancedCommentSection({ pageId }: { pageId: string }) {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      console.log("Auth user:", currentUser); // Debug: kullanıcı durumunu logla
      setUser(currentUser);
    });
  ...
  const signIn = async () => {
    try {
      ...
      console.log("Sign-in result:", result); // Debug: giriş sonucu
    } catch (error) {
      console.error("Sign-in error:", error); // Debug: giriş hatası
    }
  };
    ...
    <div className="mt-8">
      ...
      <ul>
        {comments.map((comment) => {
          console.log("comment.userId:", comment.userId, "user.uid:", user?.uid); // Debug: id karşılaştırması
          return (
...
fs.writeFileSync(commentSectionPath, updatedContent, 'utf8');
console.log(`AdvancedCommentSection.tsx dosyası debug loglarıyla güncellendi: ${commentSectionPath}`);
